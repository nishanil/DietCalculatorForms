...
{
    // In the original implementation this class was named DietCalculatorModel_v2.cs>
    // It has been renamed to DietCalculatorViewModel and modified slightly (added properties to "Gender", "SelectedGender" and "level of activites" to bind to list)
    // However majority of the code has been retained as is to show - the existing .NET code can go mobile without major modifications
    public class DietCalculatorViewModel : INotifyPropertyChanged
    {
        private int age = 0;
        ...
        private List<string> levelOfActivities;

        private int selectedGenderIndex;

        private int selectedLevelOfActivityIndex;

        public int SelectedLevelOfActivityIndex
        {
            get { return selectedLevelOfActivityIndex; }
            set { 
                selectedLevelOfActivityIndex = value;
                Activity =  (LevelOfActivity)Enum.Parse(typeof(LevelOfActivity), LevelOfActivities[selectedGenderIndex]);
            }
        }
        ...
        public int SelectedGenderIndex
        {
            ...
            set {
                ...
                if (selectedGenderIndex == 0)
                    ...
                    IsMale = false;
                OnPropertyChanged("SelectedGenderIndex");
            }
        }
        

        public DietCalculatorViewModel(Xamarin.Forms.INavigation navigation)
        {
            LevelOfActivities = Enum.GetNames(typeof(LevelOfActivity)).ToList();

            CalculateCommand = new Xamarin.Forms.Command(async () => {
                if (navigation != null)
                    await navigation.PushAsync(new ResultsPage(new ResultsViewModel() { 

                        CaloriesPerDay = CaloriesPerDay.ToString(),
                        LeanBodyMass = LeanBodyMass.ToString(),
                        PercentBodyFat = PercentBodyFat.ToString() + " %",
                        WaistHipsRatio = WaistHipsRatio.ToString() + " cm",
                        BMI = BMI.ToString(),
                        CholesterolRatio = CholesterolRatio.ToString() + " mmol/L",
                        WaistHeightRatio = WaistHeightRatio.ToString() + " cm",
                        IdealWeight = IdealWeight.ToString() + " kg"
        ...
        public List<string> LevelOfActivities
        {
            get { return levelOfActivities; }
            set { levelOfActivities = value; }
        }
